...
around the LLM:
  ...
  1. The assessment is reduced to programmatically comparable fields
     (`level`, `score`, `risk_flags` as *flag codes*, `evidence_sufficient`),
     so validators may disagree in prose but must agree on the decision.
...
def _normalize_assessment(raw: dict[str, typing.Any]) -> dict[str, typing.Any]:
    ...
    level = _level_from_name(str(raw.get("level", "NEW")))
    ...
    # Insufficient evidence can never yield a rating.
    if not sufficient:
        level = LVL_NEW
        score = min(score, 20)
...
class AgentReputation(gl.Contract):
    ...
    def _assess_with_consensus(
        ...
        Agreement is required on the *decision fields* only:
            ...
            level                -> exact on attempt 1, +/-1 band once escalated
            score                -> within the attempt's tolerance
        ...
        def _own_assessment() -> dict[str, typing.Any]:
            docs = _fetch_all()
            readable = sorted([d["url"] for d in docs if d["body"].strip() != ""])
            prompt = _build_prompt(agent_id, label, criteria, bucket, docs)
        ...
        def leader_fn() -> str:
            return json.dumps(_own_assessment())

        def validator_fn(leader_result: typing.Any) -> bool:
            # GenVM v0.2.16 passes a Result wrapper to safe run_nondet validators.
            ...
            # 4. Level: exact on the first attempt, one band of slack later.
            if abs(int(leader.get("level", -1)) - mine["level"]) > level_slack:
                return False

            # 5. Score within the attempt's tolerance.
            if abs(int(leader.get("score", -1)) - mine["score"]) > tolerance:
                return False

            # 6. Escalated attempts: the reasoning itself must be compatible.
    ...
    def is_trusted(self, agent_id: str, min_level: str = "GOOD") -> bool:
        ...
        if len(_major([f for f in agent.risk_flags.split(",") if f != ""])) > 0:
            return False
        return int(agent.level) >= int(_level_from_name(min_level))
