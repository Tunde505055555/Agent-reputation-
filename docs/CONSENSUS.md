...
level               : 0..4          exact on attempt 1, ±1 band once escalated
score               : 0..100        within the attempt's tolerance
...
`_normalize_assessment` does the reduction: clamps the score, maps the level
name, drops any flag code the model invented, and — if
`evidence_sufficient=false` — forces `NEW`, caps the score at 20 and adds
`THIN_EVIDENCE`. Thin evidence can therefore never become a good score, no matter
what the model says.
