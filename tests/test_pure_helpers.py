...
def test_level_is_derived_from_score_not_from_the_model():
    # Model claims GOOD on a FAIR-band score: the score wins.
    out = ar._normalize_assessment(
        {"score": 45, "level": "GOOD", "evidence_sufficient": True}
    )
    assert out["score"] == 45
    assert out["level"] == int(ar.LVL_FAIR)


def test_claimed_lower_level_pulls_the_score_down():
    out = ar._normalize_assessment(
...
def test_score_bands_map_to_levels():
    assert ar._level_from_score(0) == ar.LVL_NEW
    assert ar._level_from_score(19) == ar.LVL_NEW
    assert ar._level_from_score(20) == ar.LVL_LOW
    assert ar._level_from_score(59) == ar.LVL_FAIR
    assert ar._level_from_score(60) == ar.LVL_GOOD
    assert ar._level_from_score(100) == ar.LVL_TRUSTED


def test_trust_threshold_predicate_matches_is_trusted_default():
    assert not ar._meets_trust_threshold(59)
    assert ar._meets_trust_threshold(60)
